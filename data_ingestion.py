...
class FMPClient:
    ...
    def _build_request(
        ...
        """Return (url, params) tuple for the correct FMP stable endpoint."""
        params = {"symbol": symbol, "from": start, "to": end, "apikey": self._key}
        if interval == DAILY_INTERVAL:
            url = f"{self._base}/historical-price-eod/full"
        else:
            url = f"{self._base}/historical-chart/{interval}"
        return url, params
...
def _parse_response(payload: object, interval: str) -> pd.DataFrame:
    ...
    Stable API returns a bare list for both daily and intraday.
    Legacy daily endpoint wraps data in {"historical": [...]}.
    """
    if isinstance(payload, list):
        records = payload
    elif isinstance(payload, dict):
        # Legacy daily wrapper — kept for backward compatibility
        records = payload.get("historical", [])
    else:
        raise FMPError(f"Unexpected payload type: {type(payload)}")
